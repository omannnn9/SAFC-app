...
import { PLANS, planMeets, type Plan, type Feature, FEATURE_MIN_PLAN } from "@/lib/plans";
...
export function UpgradeModal({ open, onClose, currentPlan, feature, title, reason }: Props) {
  if (!open) return null;
  const need: Plan = feature ? FEATURE_MIN_PLAN[feature] : "silver";
  const target = PLANS.find((p) => p.id === need)!;
  const Icon = target.icon;

  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="glass relative w-full max-w-sm overflow-hidden rounded-3xl p-6 ring-1 ring-[var(--sa-gold)]/40">
        ...
        <div className="mt-4 rounded-2xl bg-surface-2 p-4">
          <div className="flex items-center gap-2">
            <Icon className="h-5 w-5 text-[var(--sa-gold)]" />
            <div className="font-display text-lg font-black">{target.name}</div>
            <div className="ml-auto text-sm font-bold text-muted-foreground">{target.price}</div>
          </div>
        ...
        {planMeets(currentPlan, need) ? (
          ...
          >
            <Crown className="h-4 w-4" /> Upgrade to {target.name}
          </Link>
        )}
