...
export async function enrichPosts(
  ...
  const [
    ...
    currentUserId
      ...
      : Promise.resolve({ data: [] }),
    currentUserId
      ? db.from("post_saves").select("post_id").eq("user_id", currentUserId).in("post_id", postIds)
      : Promise.resolve({ data: [] }),
  ]);

  const authorMap = new Map<string, AuthorMini>(
    ((authorsRes.data ?? []) as AuthorMini[]).map((a) => [a.id, a]),
  );
...
export async function toggleFollow(targetId: string, userId: string, currentlyFollowing: boolean) {
  if (currentlyFollowing) {
    ...
    await db
      ...
      .insert({ user_id: userId, achievement_id: "first_follow" })
      .then(
        () => {},
        () => {},
      );
...
export async function setAttendance(
    ...
    if (!wasCounted) {
      ...
      if (count >= 5)
        throw new PlanLimitError(
          "Free supporters can RSVP to up to 5 events per month. Upgrade to Basic for unlimited access.",
        );
    }
  }
  const { error } = await db
  ...
  if (status === "going") {
    await db
      .from("user_achievements")
      .insert({ user_id: userId, achievement_id: "first_event" })
      .then(
        () => {},
        () => {},
      );
...
export type EventPhoto = {
  ...
  caption: string | null;
  created_at: string;
  uploader: { full_name: string; username: string | null; avatar_url: string | null } | null;
};
...
export async function uploadEventPhoto(
  ...
  await db
    .from("event_photos")
    .insert({ event_id: eventId, user_id: userId, image_url: url, caption: caption ?? null });
  return url;
}
...
export type GroupRow = {
  id: string;
  event_id: string | null;
  type: "travel" | "meetup" | "community" | "private" | "gold";
  name: string;
  description: string | null;
  city: string | null;
  country: string | null;
...
export async function fetchGroups(
  opts: { eventId?: string; type?: GroupRow["type"]; limit?: number } = {},
): Promise<GroupRow[]> {
  let q = db
    .from("groups")
    .select("*")
...
export async function createGroup(input: Omit<GroupRow, "id" | "created_at" | "member_count">) {
  ...
  await db
    .from("group_members")
    .insert({ group_id: (data as { id: string }).id, user_id: input.owner_id, role: "owner" });
...
export async function fetchSuggestedUsers(
  ...
  const { data } = await db
    ...
    .order("created_at", { ascending: false })
    .limit(40);
  const all = ((data ?? []) as SuggestedUser[]).filter(
    (p) => p.id !== currentUserId && !myFollowing.has(p.id),
  );
  const scored = all.map((p) => {
    let score = 0;
    let reason = "Supporter";
    if (p.city && myCity && p.city === myCity) {
      score += 3;
      reason = `Same city — ${p.city}`;
    } else if (p.country && myCountry && p.country === myCountry) {
      score += 1;
      reason = `From ${p.country}`;
    }
...
export async function fetchTrendingPosts(
  ...
  const { data } = await db.from("post_likes").select("post_id").gte("created_at", since);
  const counts = new Map<string, number>();
